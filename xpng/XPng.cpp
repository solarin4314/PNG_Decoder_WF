...
#define DOERR(err)						{ASSERT(0); m_err = err; return FALSE;}
...
BOOL XPng::Decode()
{
	if(m_bEnd) {ASSERT(0); return FALSE;}
...
BOOL XPng::ProcessPalette(int dataLen)
{
	...
	if(m_palette==NULL)
	{
		m_palette = (BYTE*)malloc(dataLen);
		m_trans = (BYTE*)malloc(LEN_TRANS);
	}

	// 읽기
	if(m_pStream->Read(m_palette, dataLen)==FALSE)
		DOERR(XPNG_ERR_CANT_READ);

#ifndef _XPNG_NO_CRC
	// crc 업데이트
	m_crc32 = CRC32(m_crc32, m_palette, dataLen);
#endif
